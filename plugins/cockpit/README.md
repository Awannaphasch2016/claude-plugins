...
| lines 5–9 `lap` · `race` · `stint` · `lab` · `ctx` | instrument | dim, no colour, no verdict. **One compact = one lap; the session = the race; compacting = boxing.** `lap`/`race` are equal-width mix bars (assistant ▰ / tool ▱ / you ▷ — composition in the bar, size in the `t` count, exact % in the labels); `stint` is compact cadence (sparkline of closed-lap lengths, this lap vs last, median); `lab` dials (`areas · old · dup · tool · files`) and the `ctx` liveness strip are **lap-scoped** — a race-scope numerator over a lap-scope denominator is how `tool 616%` happened. A `box` line appears for the first ~5 turns after a compact: the closed lap is judgeable, so `/felt` it — rain or planned? Closed laps archive to `laps.<hash>.json` for after the race |

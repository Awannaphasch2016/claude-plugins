...
| `goal` | instrument | the work-graph pace line — renders `pace.json` (a cache the lens dual-writes; the statusline never computes graph facts): goal · time-to-line · slack bar · slack recomputed TIME-FRESH every render · required rate + trend arrow (↗ = lock-in approaching) · head item + turns poured since the read · the provenance stamp `(run N · age)`, ⚠ past 24h — the fresh-vs-stale law on one token |
| `lap`+`dials`+`ctx` · `race`+`stint` | instrument | dim, no colour, no verdict. **One compact = one lap; the session = the race; compacting = boxing.** `lap`/`race` are equal-width mix bars (assistant ▰ / tool ▱ / you ▷ — composition in the bar, size in the `t` count, exact % in the labels); `stint` is compact cadence (sparkline of closed-lap lengths, this lap vs last, median); the `dials` (`areas · old · dup · tool · files`) and `ctx` liveness strip sit INDENTED UNDER `lap` — grouped by scope, and renamed from `lab` because lab-vs-lap was two meanings one letter apart — and are **lap-scoped** — a race-scope numerator over a lap-scope denominator is how `tool 616%` happened. A `box` line appears for the first ~5 turns after a compact: the closed lap is judgeable, so `/felt` it — rain or planned? Closed laps archive to `laps.<hash>.json` for after the race |
| line 7 `prefix` | alarm, conditional | `❄ idle Nm (lease expired) — next turn likely rebuilds ~Xk` — the free moment to compact or clear |
| PreCompact hook | — | the /felt moment caught BEFORE the box: one `systemMessage` nudge when you `/compact` ("boxing lap N (~Nt) — rain or planned?"), worded differently on auto-compact (the tank ran dry — no choice was made, which is itself a signal). Never a blocking dialog: /felt is the driver's ground truth |
